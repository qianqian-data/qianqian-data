...
Aspiring Data Analyst transitioning from international publishing and content licensing. I build data projects that go beyond analysis — turning insights into actionable decisions and working products. Open to any data analyst role where analytical thinking drives real business impact.
...
- Key finding: Energy sector carries 2.8× higher ESG risk than Real Estate; carbon transition risk is systematically underestimated in official ratings
- [GitHub](https://github.com/qianqian-data/ESG-risk-dashboard) · [📊 Live Dashboard](https://public.tableau.com/app/profile/qianqian.z7250/viz/ESGRiskDashboardSP500Sustainalytics-InspiredModel/ESGRiskDashboard#1)

## Skills
`Python` `SQL` `pandas` `scikit-learn` `Tableau` `Power BI` `matplotlib` `seaborn` `Streamlit` `Plotly` `Claude API` `Git`

## Background
Previously a Rights Executive at a French comics publisher, managing content licensing across Asian markets. Trilingual (Chinese / French C2 / English C1), with a strong interest in ESG data, sustainable finance, and any domain where data analysis drives meaningful decisions.
